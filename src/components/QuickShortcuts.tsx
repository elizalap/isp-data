...
export function QuickShortcuts({
  ...
  const cards = [
    {
      key: "celeti",
      label: "Não-clientes Celeti",
      color: "var(--blue)",
      count: data?.filter((p) => !p.celeti).length,
    },
    {
      key: "hub",
      label: "Não-clientes Hub",
      color: "var(--teal)",
      count: data?.filter((p) => !p.hub).length,
    },
    {
      key: "cdn",
      label: "Não-clientes CDN",
      color: "var(--coral)",
      count: data?.filter((p) => !p.cdn).length,
    },
    {
      key: "rami",
      label: "Não-clientes Rami",
      color: "var(--amber)",
      count: data?.filter((p) => !p.rami).length,
    },
    {
      key: "any",
      label: "Sem nenhum produto",
      color: "var(--text2)",
      count: data?.filter((p) => !p.celeti && !p.hub && !p.cdn && !p.rami).length,
    },
  ];
  return (
    <div

...
const NAME_TO_UF = Object.fromEntries(
  Object.entries(UF_NAME).map(([k, v]) => [v.toLowerCase(), k]),
);
...
export function BrazilMap({
  ...
  const stats = useMemo(() => {
    ...
    const asnByPtt: Record<
      string,
      { count: number; ufs: { uf: string; count: number; km: number }[] }
    > = {};
  ...
  const metricLabel =
    metric === "asn"
      ? "ASNs conectados por PTT IX.br"
      : metric === "cdn"
        ? "Potencial CDN (sem CDN)"
        : "Total de provedores";
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button
            style={btnStyle(metric === "total", "#4F8EF7")}
            onClick={() => setMetric("total")}
          >
            Total
          </button>
          <button style={btnStyle(metric === "asn", "#3DD68C")} onClick={() => setMetric("asn")}>
            ASN × PTT
          </button>
          <button style={btnStyle(metric === "cdn", "#F7A84F")} onClick={() => setMetric("cdn")}>
            Potencial CDN
          </button>
        </div>
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: metric === "asn" ? "1fr 260px" : "1fr",
          gap: 16,
          alignItems: "start",
        }}
      >
        <div ref={ref} style={{ position: "relative", width: "100%" }}>
          ...
          {geo && path && (
            <svg width={w} height={h} style={{ display: "block" }}>
              ...
              {projection &&
                metric === "asn" &&
                Object.entries(stats.asn).map(([uf]) => {
              ...
              {projection &&
                metric === "asn" &&
                Object.entries(stats.asn).map(([uf, count]) => {
              ...
              {projection &&
                PTTS.filter(
                  (p) =>
                    PTT_HIGHLIGHT.includes(p.name) ||
                    (metric === "asn" && stats.asnByPttSorted.find((s) => s.name === p.name)),
                ).map((p) => {
                  const c = projection([p.lon, p.lat]);
                  if (!c) return null;
                  const fill = metric === "asn" ? pttColor(p.name) : "#F76F6F";
                  const dim =
                    highlightPtt && metric === "asn" && p.name !== highlightPtt ? 0.25 : 1;
                  return (
                    <g
                      key={p.name}
                      ...
                      onMouseMove={(e) => {
                        ...
                        const total =
                          stats.asnByPttSorted.find((s) => s.name === p.name)?.count || 0;
                        setHover({
                          x: e.clientX - rect.left + 12,
                          y: e.clientY - rect.top + 12,
                      ...
                      onMouseLeave={() => {
                        setHover(null);
                        if (metric === "asn") setHighlightPtt(null);
                      }}
                      onClick={() => {
        ...
        {metric === "asn" && (
          ...
          >
            ...
            >
              Atribuição feita pela <b>menor distância haversine</b> entre a UF e os PTTs IX.br — é
              uma aproximação do peering provável, não do AS-PATH real.
            </div>
          </div>
        )}
      </div>
      ...
      >
        ...
        <div
          style={{
            ...
            height: 8,
            borderRadius: 4,
            background: `linear-gradient(to right, ${colorFn(0)}, ${colorFn(max)})`,
          }}
        />
        <span>
          {max.toLocaleString("pt-BR")} {metric === "asn" ? "ASNs/UF" : "provedores/UF"}
        </span>
        {metric === "asn" && (
          <>
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 18, borderTop: "1.5px dashed #9CA3AF" }} /> ASN → PTT (cor do
              PTT)
            </span>
          </>
        )}

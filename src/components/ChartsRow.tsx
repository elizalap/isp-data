...
export function ChartsRow({ providers }: { providers: Provider[] }) {
  ...
  const erp = useMemo(() => {
    const c: Record<string, number> = {};
    providers.forEach((p) => {
      if (p.erp && !/sem erp/i.test(p.erp)) c[p.erp] = (c[p.erp] || 0) + 1;
    });
    return Object.entries(c)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
        ...
        <Card title="Status de e-mail" dot="var(--coral)">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={emailDist} layout="vertical" margin={{ left: 8, right: 28 }}>
              ...
              <Bar dataKey="value" radius={[0, 3, 3, 0]}>
                {emailDist.map((d, i) => (
                  <Cell key={i} fill={emailColorOf(d.name)} />
                ))}
                <LabelList
                  dataKey="value"
                  position="right"
            ...
            <div key={c.label}>
              ...
              >
                ...
                <span style={{ color: "var(--text)", fontWeight: 600 }}>
                  {c.pct}%{" "}
                  <span style={{ color: "var(--text3)" }}>({c.n.toLocaleString("pt-BR")})</span>
                </span>
              </div>
              <div
              ...
              >
                <div
                  style={{
                    ...
                    height: "100%",
                    background:
                      c.pct > 70 ? "var(--green)" : c.pct > 40 ? "var(--amber)" : "var(--coral)",
                  }}
                />
              </div>
            </div>

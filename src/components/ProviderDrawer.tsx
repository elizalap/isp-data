...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <span
        style={{ color: "var(--text)", fontSize: 13, textAlign: "right", wordBreak: "break-word" }}
      >
        {value}
      </span>
    </div>
...
function emailStatusColor(kind: "valido" | "invalido" | "inconclusivo" | "sem") {
  ...
  if (kind === "inconclusivo")
    return { c: "var(--amber)", bg: "var(--amber-dim)", t: "Inconclusivo" };
...
export function ProviderDrawer({ p, onClose }: { p: Provider | null; onClose: () => void }) {
  ...
  const phones = [p.tel, p.tel2, p.tel3, p.tel4, p.tel5]
    .map(formatPhone)
    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
          <button onClick={copyContact} style={actionBtn}>
            📋 Copiar contato
          </button>
          <button onClick={exportRow} style={actionBtn}>
            ⬇ Exportar linha
          </button>
        </div>
        ...
        />
        {phones.map((t, i) => (
          <Row key={i} label={`Telefone ${i + 1}`} value={t} />
        ))}
        ...
        <ul style={{ paddingLeft: 18, margin: 0 }}>
          {recs.map((r, i) => (
            <li
              key={i}
              style={{ fontSize: 12.5, color: "var(--text)", marginBottom: 8, lineHeight: 1.5 }}
            >
              {r}
            </li>
          ))}

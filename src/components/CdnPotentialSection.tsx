...
export function CdnPotentialSection({
  providers,
  onSelect,
}: {
  providers: Provider[];
  ...
  const ufs = useMemo(
    () => [...new Set(enriched.map((r) => r.uf).filter(Boolean) as string[])].sort(),
    [enriched],
  );
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortKey === "km") {
        ...
        va = (a.fantasia || a.nome || "").toLowerCase();
        vb = (b.fantasia || b.nome || "").toLowerCase();
      } else if (sortKey === "uf") {
        va = a.uf || "";
        vb = b.uf || "";
      } else {
  ...
  const counts = useMemo(() => {
    let alta = 0,
      ...
      baixa = 0;
    enriched.forEach((r) => {
      if (r._prio === "alta") alta++;
      else if (r._prio === "media") media++;
      else baixa++;
    });
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 12, color: "var(--text2)", marginBottom: 14 }}>
        Distância geográfica ao IX.br mais próximo como proxy de necessidade de CDN. Maior distância
        = maior ganho potencial de performance e latência para o usuário final. Considera UF do
        provedor x localização dos PTTs.
      </p>
      ...
      >
        <div style={{ display: "flex", gap: 4 }}>
          <Tab active={tab === "alta"} onClick={() => setTab("alta")}>
            Alta &gt;1.000 km <Pill>{counts.alta.toLocaleString("pt-BR")}</Pill>
          </Tab>
          <Tab active={tab === "media"} onClick={() => setTab("media")}>
            Média 500–1.000 km <Pill>{counts.media.toLocaleString("pt-BR")}</Pill>
          </Tab>
          <Tab active={tab === "baixa"} onClick={() => setTab("baixa")}>
            Baixa &lt;500 km <Pill>{counts.baixa.toLocaleString("pt-BR")}</Pill>
          </Tab>
          <Tab active={tab === "todos"} onClick={() => setTab("todos")}>
            Todos <Pill>{counts.total.toLocaleString("pt-BR")}</Pill>
          </Tab>
        </div>
        <input
          ...
          <thead>
            <tr>
              <Th
                ...
                arrow={sortKey === "nome" ? (asc ? "↑" : "↓") : ""}
              >
                Provedor
              </Th>
              <Th
                onClick={() => toggleSort("uf")}
                arrow={sortKey === "uf" ? (asc ? "↑" : "↓") : ""}
              >
                UF
              </Th>
              <Th
                onClick={() => toggleSort("municipio")}
                arrow={sortKey === "municipio" ? (asc ? "↑" : "↓") : ""}
              >
                Município
              </Th>
          ...
          <tbody>
            {shown.map((r, i) => {
              ...
              return (
                <tr
                  ...
                  onMouseEnter={(e) =>
                    (e.currentTarget.style.background = "rgba(247,111,111,0.05)")
                  }
                ...
                >
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ minWidth: 44, fontWeight: 600, color }}>
                        {r._km.toLocaleString("pt-BR")}
                      </span>
                      <div
                        style={{
                          flex: 1,
            ...
            {shown.length === 0 && (
              <tr>
                <td
                  colSpan={7}
                  style={{ ...tdStyle, textAlign: "center", color: "var(--text3)", padding: 32 }}
                >
      ...
      <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 10 }}>
        Exibindo {shown.length.toLocaleString("pt-BR")} de {filtered.length.toLocaleString("pt-BR")}{" "}
        potenciais filtrados · {counts.total.toLocaleString("pt-BR")} não-clientes CDN na base atual
      </div>
    </div>
  );
}
...
function Th({
  ...
  return (
    ...
    >
      {children} {arrow && <span style={{ color: "var(--blue)" }}>{arrow}</span>}
    </th>
  );
}
...
const tdStyle: React.CSSProperties = {
  padding: "10px",
  borderBottom: "1px solid rgba(38,48,73,0.5)",
  color: "var(--text)",

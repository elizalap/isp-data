...
function Dashboard() {
  ...
  const [ixbrF, setIxbrF] = useState<"" | "sim" | "nao">("");
  const [pttQtdF, setPttQtdF] = useState<"" | "0" | "1" | "2-5" | "5+">("");
  const [peeringF, setPeeringF] = useState("");
  ...
  const dims = useMemo(() => {
    if (!data) return { ufList: [], portes: [], erps: [], peerings: [] };
    ...
    data.forEach((r) => {
      if (r.uf) u.add(r.uf);
      if (r.porte) pr.add(r.porte);
      if (r.erp) e.add(r.erp);
      if (r.politica_peering) pe.add(r.politica_peering);
    });
    return {
      ufList: [...u].sort(),
      portes: [...pr].sort(),
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((r) => {
      ...
      if (faixaAss) {
        ...
        const bucket = ASSINANTES_BUCKETS.find((b) => b.value === faixaAss);
        if (!bucket || a < 0 || !bucket.match(a)) return false;
      }
      if (maturF) {
      ...
      if (ixbrF === "sim" && !r.conectado_ixbr) return false;
      if (ixbrF === "nao" && r.conectado_ixbr) return false;
      if (pttQtdF) {
        const q2 = r.qtd_ptts_total ?? 0;
        if (pttQtdF === "0" && q2 !== 0) return false;
        if (pttQtdF === "1" && q2 !== 1) return false;
        if (pttQtdF === "2-5" && !(q2 >= 2 && q2 <= 5)) return false;
        if (pttQtdF === "5+" && !(q2 > 5)) return false;
      }
      if (peeringF && (r.politica_peering || "") !== peeringF) return false;
      if (q) {
        const hay =
          `${r.nome || ""} ${r.fantasia || ""} ${r.municipio || ""} ${r.cnpj || ""}`.toLowerCase();
  ...
  function exportLeads() {
    const rows = filtered
      .filter((p) => {
        ...
        return (
          (s === "alto" || s === "medio") && p.email && emailStatusKind(p.email_status) === "valido"
        );
      })
      .map((p) => {
        const np = nearestPTT(p.uf);
  ...
  return (
    <div>
      ...
      <main style={{ maxWidth: 1400, margin: "0 auto", padding: "24px 24px 60px" }}>
        <h1
          style={{ fontFamily: "var(--font-head)", fontSize: 26, fontWeight: 700, marginBottom: 4 }}
        >
          Mercado ISP Brasil
        </h1>
        <p style={{ fontSize: 13, color: "var(--text2)", marginBottom: 20 }}>
          Painel estratégico de inteligência competitiva, prospecção e consultoria comercial
        </p>

        {error && (
          <div style={{ color: "var(--coral)", marginBottom: 16 }}>
        ...
        <div style={filtersWrap}>
          ...
          <FG label="Cliente de">
            <select
              style={selectStyle}
              value={produto}
              onChange={(e) => setProduto(e.target.value as ProductFilter)}
            >
          ...
          <FG label="Não-cliente">
            <select
              style={selectStyle}
              value={potencial}
              onChange={(e) => setPotencial(e.target.value as PotentialFilter)}
            >
              <option value="">—</option>
          ...
          <FG label="Prioridade CDN">
            <select
              style={selectStyle}
              value={prioCdn}
              onChange={(e) => setPrioCdn(e.target.value as any)}
            >
          ...
          <FG label="Assinantes">
            <select
              style={selectStyle}
              value={faixaAss}
              onChange={(e) => setFaixaAss(e.target.value)}
            >
              <option value="">Todas</option>
              {ASSINANTES_BUCKETS.map((b) => (
          ...
          <FG label="ASN">
            <select
              style={selectStyle}
              value={asnF}
              onChange={(e) => setAsnF(e.target.value as any)}
            >
              <option value="">Todos</option>
          ...
          <FG label="Conectado IX.br">
            <select
              style={selectStyle}
              value={ixbrF}
              onChange={(e) => setIxbrF(e.target.value as any)}
            >
              <option value="">Todos</option>
              <option value="sim">Conectado</option>
              <option value="nao">Não conectado</option>
            </select>
          </FG>
          <FG label="Qtd. PTTs">
            <select
              style={selectStyle}
              value={pttQtdF}
              onChange={(e) => setPttQtdF(e.target.value as any)}
            >
              <option value="">Todas</option>
              <option value="0">Nenhum</option>
              <option value="1">1 PTT</option>
              <option value="2-5">2–5 PTTs</option>
              <option value="5+">&gt; 5 PTTs</option>
            </select>
          </FG>
          {dims.peerings.length > 0 && (
            <FG label="Política Peering">
              <select
                style={selectStyle}
                value={peeringF}
                onChange={(e) => setPeeringF(e.target.value)}
              >
                <option value="">Todas</option>
                {dims.peerings.map((p) => (
                  <option key={p} value={p}>
          ...
          <FG label="Buscar">
            <input
              ...
              placeholder="Nome, município, CNPJ..."
              value={busca}
          ...
          <div style={{ display: "flex", gap: 8, marginLeft: "auto", alignItems: "center" }}>
            <span style={{ fontSize: 12, color: "var(--text2)" }}>
              <b style={{ color: "var(--blue)" }}>{filtered.length.toLocaleString("pt-BR")}</b>{" "}
              provedores na visão
            </span>
            ...
            <button
              onClick={exportVisao}
              style={btnPrimary}
              title="Exportar todos os provedores filtrados"
            >
            ...
            <button
              onClick={exportLeads}
              style={btnGhost}
              title="Exportar apenas leads Alto/Médio com e-mail válido"
            >
              <FileDown size={13} /> Leads
            </button>
          </div>
        </div>
        ...
        >
          ...
          <Kpi
            ...
            label="Score Alto"
            value={kpis.alto}
            sub={pct(kpis.alto, kpis.total)}
          />
          <Kpi
            icon={<Building2 size={14} />}
            color="var(--blue)"
            label="Clientes Celeti"
            value={kpis.ce}
            sub={pct(kpis.ce, kpis.total)}
          />
          <Kpi
          ...
          <Kpi
            ...
            label="Com e-mail"
            value={kpis.em}
            sub={pct(kpis.em, kpis.total)}
          />
          <Kpi
            icon={<Users size={14} />}
            color="var(--text3)"
            label="Sem nenhum produto"
            value={kpis.naoCliente}
            sub={pct(kpis.naoCliente, kpis.total)}
          />
        </div>
...
function Kpi({
  ...
  return (
    ...
    >
      <div
        style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: color }}
      />
      <div
        style={{
          display: "flex",
  ...
  return (
    <div ref={ref} style={{ position: "relative", minWidth: 130 }}>
      <button
        ...
        style={{ ...selectStyle, width: "100%", textAlign: "left", cursor: "pointer" }}
      >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          style={{ height: 40, background: "var(--bg3)", borderRadius: 6, opacity: 0.5 - i * 0.06 }}
        />
      ))}
    </div>
  );

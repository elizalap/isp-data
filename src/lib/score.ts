...
export function abordagemRecomendada(p: Provider): string[] {
  ...
  if (!p.cdn && km > 800) {
    tips.push(
      `Forte candidato a CDN — ${km} km até o PTT IX.br mais próximo impacta latência dos usuários finais. Abordagem técnica recomendada.`,
    );
  }
  if (!p.hub && ass > 500) {
    tips.push(
      `Porte operacional (${ass.toLocaleString("pt-BR")} assinantes) compatível com Celeti Hub. Explorar centralização de roteamento.`,
    );
  }
  if (!p.rami && PORTE_MG.test(porte)) {
    tips.push(
      `Perfil financeiro (porte ${porte}) adequado para Rami Capital. Verificar necessidade de crédito para expansão.`,
    );
  }
  if (!p.celeti && (p.hub || p.cdn || p.rami)) {
    tips.push(
      `Já é cliente Supercomm em outro produto — oportunidade de cross-sell para Celeti (ERP).`,
    );
  }
  if (!tips.length)
    tips.push(
      "Sem gatilho automático evidente. Avaliar manualmente porte, ERP e maturidade do provedor.",
    );
  return tips;
}
...
export function maturidadeAnos(abertura: string | null): number | null {
  ...
  const m =
    String(abertura).match(/(\d{4})-(\d{1,2})-(\d{1,2})/) ||
    String(abertura).match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/);
  if (!m) return null;
...
export function emailStatusKind(
  s: string | null | undefined,
): "valido" | "invalido" | "inconclusivo" | "sem" {
  if (!s) return "sem";
